		...
		const forms = (formsSelector) => {
			...
			form.forEach(item => {
				...
				function formAddErrorEmail(input) {
					let div = document.createElement('div');
					div.classList.add("form__error");
					div.innerHTML = "Wpisz swój adres e-mail";
	...
	//----------------------ofertaItem-JS----------------------
		const ofertaItem = (ofertaItemSelector) => {
			...
			ofertaItem.forEach(item => {
				item.addEventListener('mouseenter', (e) => { 
					const ofertaItemActive = document.querySelector('.oferta__item--active');
					ofertaItemActive.classList.remove('oferta__item--active');
					item.classList.add('oferta__item--active');
				});
			});
		};
